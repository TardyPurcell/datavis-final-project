...
def getKmeansRes(list:list[models.Song,models.Song],k:int):
    ...
    joblib.dump(km, './data/doc_cluster.pkl') #第一次运行时将注释打开，项目中会生成doc_cluster.pkl文件，之后运行的时候再注释掉这行就可以使用之前持久化的模型了
    ...
    for ind in range(len(list)):
        ...
        for i in order_centroids[clusternum, :6]: #replace 3 with n words per cluster
            ...
            print('%s' %vocab_frame.loc[terms[i].split(' ')].values.tolist()[0][0].encode('utf-8', 'ignore'), end=', ')
            clustername=clustername+str(vocab_frame.loc[terms[i].split(' ')].values.tolist()[0][0].encode('utf-8', 'ignore'))
        print()
        print(clustername)
        print()
        wholike=whos[ind]
        ...
        if clusternum not in cluset:
            ...
            clustername=''
            for i in order_centroids[clusternum, :6]: #replace 6 with n words per cluster
                #b'...' is an encoded byte string. the unicode.encode() method outputs a byte string that needs to be converted back to a string with .decode()
                print('%s' %vocab_frame.loc[terms[i].split(' ')].values.tolist()[0][0].encode('utf-8', 'ignore'), end=', ')
                clustername=clustername+str(vocab_frame.loc[terms[i].split(' ')].values.tolist()[0][0].encode('utf-8', 'ignore'))
            print()
